...
1. **Write a SQL query to retrieve all columns for sales made on '2022-07-09**:
```sql
SELECT *
FROM retail_sales
...
2. **Write a SQL query to retrieve all transactions where the category is 'Clothing' and the quantity sold is more than 4 in the month of Feb-2022**:
```sql
SELECT 
  *
FROM retail_sales
WHERE 
    category = 'Clothing'
    AND 
    TO_CHAR(sale_date, 'YYYY-MM') = '2022-02'
    AND
    quantity >= 4
```
...
ORDER BY 1
```
7. **Write a SQL query to find the total number of transactions (transaction_id) made by each gender**:
```sql
SELECT 
    gender,
    COUNT(*) as total_trans
...
- **Customer Demographics**: The dataset includes customers from various age groups, with sales distributed across different categories such as Clothing and Electronics.
